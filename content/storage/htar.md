---
source: https://docs.rcac.purdue.edu/userguides/gilbreth/storage/htar/
section: storage
slug: storage/htar
title: HTAR
---
...
HTAR (short for "HPSS TAR") is a utility program that writes TAR-compatible archive files directly onto Gilbreth, without having to first create a local file. Its command line was originally based on `tar`, with a number of extensions added to provide extra features.
...
Create a tar archive on Gilbreth named data.tar including all files with the extension ".fits":
...
HTAR Create complete for data.tar. 5,120,006,144 bytes written for 5 member files, max threads: 3
Transfer time: 16.457 seconds (311.121 MB/s)
HTAR: HTAR SUCCESSFUL
```

Unpack a tar archive on Gilbreth named data.tar into a scratch directory for use in a batch job:
...
HTAR: Extract complete for data.tar, 5 files.
total bytes read: 5,120,004,608 in 18.841 seconds (271.749 MB/s )
HTAR: HTAR SUCCESSFUL
```

Look at the contents of the data.tar HTAR archive on Gilbreth:
...
Unpack a single file, `data5.fits`, from the tar archive on Gilbreth named data.tar into a scratch directory:

```
$ htar -xvf data.tar data5.fits
HTAR: x data5.fits, 1024000000 bytes, 2000001 media blocks
HTAR: Extract complete for data.tar, 1 files.
total bytes read: 1,024,000,512 in 3.642 seconds (281.166 MB/s )
HTAR: HTAR SUCCESSFUL
```
...
- Lawrence Livermore National Labs HTAR User Guide
