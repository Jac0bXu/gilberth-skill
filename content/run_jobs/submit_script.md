---
source: https://docs.rcac.purdue.edu/userguides/gilbreth/run_jobs/submit_script/
section: run_jobs
slug: run_jobs/submit_script
...
```bash
sbatch -t 1:30:00 --mem=8G --nodes=1 --gpus-per-node=1 -A accountname --partition=a10 --ntasks=4 myjobsubmissionfile
```
...
After submitting a job, you can monitor its status, wait for it to finish, and then check its output.
